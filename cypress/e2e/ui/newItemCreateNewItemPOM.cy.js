...
describe('US_00.000 | New Item > Create New item', () => {
  ...
  it('TC_00.000.05 | Create new item from Dashboard dropdown menu', () => {
    dashboardPage.hoverDashboardDropdownChevron().clickDashboardDropdownChevron().selectNewItemFromDashboardChevron()
    newJobPage.typeNewItemName(randomItemName).selectFreestyleProject().clickOKButton()
    header.clickJenkinsLogo()
  ...
  it('TC_00.000.06 | Create new item from the "New Item" link in the left sidebar', () => {
    ...
    dashboardPage.getJobTable().contains(randomItemName).should('be.visible').and('have.text', randomItemName)
  })
  ...
  it('TC_00.000.07 | Verify new item can only be created using unique item names', () => {
    ...
    newJobPage
      ...
      .should('contain.text', `${newItem.duplicateNotAllowedMessage} ‘${randomItemName}’`)
    newJobPage.clearItemNameField().typeNewItemName(newRandomItemName).selectFreestyleProject().clickOKButton()
    header.clickJenkinsLogo()

    cy.log('Verifying that new item with unique name was created')
    dashboardPage.getJobTable().contains(newRandomItemName).should('be.visible').and('have.text', newRandomItemName)
  })
  ...
  it('TC_00.000.08 | Verify item name does not contain any special characters', () => {
    ...
    newJobPage.clearItemNameField().typeNewItemName(projectName).selectFreestyleProject().clickOKButton()
    header.clickJenkinsLogo()
  ...
  it('TC_00.000.10 | Verify that to create a new item, user should choose "an item type" first', () => {
    dashboardPage.clickNewItemMenuLink()
    newJobPage.typeNewItemName(randomItemName).getOKButton().should('be.disabled')
  ...
  it('TC_00.000.14 | Verify the display of validation message if no item name is entered', () => {
    dashboardPage.clickNewItemMenuLink()
    newJobPage.typeNewItemName(randomItemName).clearItemNameField()

    newJobPage.getEmptyItemInvalidName().should('contain.text', newItem.emptyNameFieldReminder)
  })
